...
CREATE USER u1_01292 IDENTIFIED WITH plaintext_password BY 'qwerty';
CREATE USER u1_01292 IDENTIFIED WITH sha256_password BY 'qwerty';
CREATE USER u1_01292 IDENTIFIED BY 'password';
CREATE USER u1_01292 IDENTIFIED WITH sha256_hash BY 'hash';
CREATE USER u1_01292 IDENTIFIED WITH double_sha1_password BY 'qwerty';
CREATE USER u1_01292 IDENTIFIED SAN 'TYPE:subject_alt_name';
CREATE USER u1_01292 IDENTIFIED WITH double_sha1_hash BY 'hash';
CREATE USER u1_01292 IDENTIFIED WITH bcrypt_password BY 'qwerty';
CREATE USER u1_01292 IDENTIFIED WITH bcrypt_hash BY 'hash';
CREATE USER u1_01292 IDENTIFIED WITH ldap SERVER 'server_name';
CREATE USER u1_01292 IDENTIFIED WITH ssl_certificate CN 'mysite.com:user';
CREATE USER u1_01292 IDENTIFIED WITH ssh_key BY KEY 'public_key' TYPE 'ssh-rsa';
CREATE USER u1_01292 IDENTIFIED WITH http SERVER 'http_server';
CREATE USER u1_01292 IDENTIFIED WITH http SERVER 'http_server' SCHEME 'basic';
CREATE USER u1_01292 IDENTIFIED BY 'qwerty';
CREATE USER OR REPLACE ldap_user 
IDENTIFIED WITH ldap SERVER 'ad_server'
HOST IP '192.168.1.0/24'
VALID UNTIL '2024-12-31'
DEFAULT ROLE readonly;
CREATE USER web_app
IDENTIFIED WITH http SERVER 'auth_proxy' SCHEME 'Basic'
HOST LIKE '%.web-servers.%'
DEFAULT DATABASE app_data;
CREATE USER reporting_user
ON CLUSTER cluster_1
IDENTIFIED WITH double_sha1_hash BY 'hash_value'
HOST NAME 'report-server'
DEFAULT ROLE report_viewer
SETTINGS PROFILE 'reporting_profile';
...
ALTER USER user7 ADD SETTINGS max_memory_usage=5000000 WRITABLE;
ALTER USER user7 MODIFY SETTINGS max_memory_usage=5000000 WRITABLE;
...
ALTER USER u1_01292 NOT IDENTIFIED HOST LIKE '%.%.myhost.com' DEFAULT ROLE NONE ADD PROFILES 'default';
ALTER USER u1_01292 SETTINGS readonly=1;
ALTER USER rabbit, fox ADD SETTINGS readonly=1;
ALTER USER tom_riddl RENAME TO volandemort MODIFY SETTINGS WRITABLE=1;
ALTER USER cat DROP ALL SETTINGS;
ALTER USER cat_2 DROP ALL PROFILES;
ALTER USER cat_3 DROP PROFILES tom, murzik;
...
ALTER USER student, worker MODIFY SETTINGS readonly=1, ADD SETTINGS MIN = 3 MAX= 6;
