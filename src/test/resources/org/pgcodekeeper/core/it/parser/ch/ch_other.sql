...
ATTACH TABLE test_table AS REPLICATED;
ATTACH TABLE test_table AS NOT REPLICATED;
ATTACH TABLE IF NOT EXISTS test_db.test_table AS REPLICATED;
CHECK ALL TABLES;
...
USE test_db;
RENAME test_old TO test_new;
RENAME test_old TO test_new ON CLUSTER my_cluster;
TRUNCATE TABLE test_table SYNC;
TRUNCATE ALL TABLES FROM test_db;
TRUNCATE TABLES FROM test_db;
TRUNCATE ALL TABLES FROM IF EXISTS test_db;
TRUNCATE TABLES FROM IF EXISTS test_db LIKE 'test%';
TRUNCATE TABLES FROM test_db LIKE 'test%' ON CLUSTER my_cluster;
TRUNCATE DATABASE IF EXISTS test_db;
TRUNCATE DATABASE test_db ON CLUSTER my_cluster;
EXECUTE AS admin;
EXECUTE AS admin SELECT * FROM test_table;
ALTER DATABASE test_db MODIFY COMMENT 'Test database';
ALTER DATABASE test_db ON CLUSTER my_cluster MODIFY COMMENT 'Test database';
SELECT 1 PARALLEL WITH SELECT 2;
SELECT * FROM t1 PARALLEL WITH SELECT * FROM t2;
SELECT * FROM t1 PARALLEL WITH SELECT * FROM t2 PARALLEL WITH SELECT * FROM t3;