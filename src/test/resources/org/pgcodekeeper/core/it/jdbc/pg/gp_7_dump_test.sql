...
CREATE FOREIGN DATA WRAPPER wrap OPTIONS (debug 'true');
...
COMMENT
ON RULE r3 ON public.test IS 'test comment';

-- test policy

CREATE POLICY p1 ON public.test AS RESTRICTIVE FOR SELECT TO gpadmin USING (true);
CREATE POLICY p2 ON public.test AS PERMISSIVE FOR INSERT TO gpadmin WITH CHECK ((1 > 0));
CREATE POLICY p3 ON public.test TO gpadmin USING (true) WITH CHECK ((1 > 0));
...
(
    ...
    c2 text,
    c3 bigint
) PARTITION BY LIST ("left"(lower(c2), 1)) DISTRIBUTED BY (c1);

ALTER TABLE public.base OWNER TO gpadmin;
...
ALTER TABLE public.child_ab OWNER TO gpadmin;
...
ALTER FOREIGN TABLE public.child_xz OWNER TO gpadmin;
