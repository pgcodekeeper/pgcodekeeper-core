...
CREATE VIEW public.gpadmin AS
   ...
   FROM (( SELECT v.id AS base_id,
           ...
           FROM ( SELECT st.c1 AS id,
                        CASE
                            WHEN (public.f1('second'::text, st.c2) > 0) THEN 1
                            ELSE 0
                        END AS c2,
                    st.c3,
                    st.c4,
          ...
          GROUP BY v.id, v.c2, v.c3, v.c4, v.c5) t
     LEFT JOIN public.t1 tx ON ((t.base_id = tx.id)));
...
ALTER FUNCTION public.f1(p1 text, p2 integer) OWNER TO gpadmin;

ALTER FUNCTION public.f2() OWNER TO gpadmin;

ALTER VIEW public.v1 OWNER TO gpadmin;

ALTER VIEW public.v2 OWNER TO gpadmin;

ALTER VIEW public.gpadmin OWNER TO gpadmin;