...
/*FOR ...(SELECT...) ...LOOP*/
    for v_next_sync in (select sys_change_version from bankrupt_bulk.address order by 1 )
    loop
    end loop;
...
/*MERGE*/
    MERGE INTO customer_account ca USING (SELECT customer_id, transaction_value FROM recent_transactions) AS t ON t.customer_id = ca.customer_id
    WHEN MATCHED THEN
