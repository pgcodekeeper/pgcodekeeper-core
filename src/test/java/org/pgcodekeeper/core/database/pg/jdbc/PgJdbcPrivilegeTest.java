...
class PgJdbcPrivilegeTest {

    @Test
    void testParse() {
        String order = "CU";
        String acl = "{main=UC/main,user=U*/main,guest=C/main,=UC/main}";
        List<PgJdbcPrivilege> privs = PgJdbcPrivilege.parse(acl, order, null);
        assertEquals(4, privs.size());

        PgJdbcPrivilege first = privs.get(0);
        ...
        PgJdbcPrivilege second = privs.get(1);
        ...
        PgJdbcPrivilege third = privs.get(2);
        ...
        PgJdbcPrivilege fourth = privs.get(3);
    ...
    void testParseQuotedGrantee(String acl, String owner, String expected) {
        String order = "CU";
        List<PgJdbcPrivilege> privs = PgJdbcPrivilege.parse(acl, order, owner);

        PgJdbcPrivilege priv = privs.get(0);
