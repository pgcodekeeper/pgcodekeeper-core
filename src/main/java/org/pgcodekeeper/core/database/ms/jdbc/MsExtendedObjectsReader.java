 ...
 */
public class MsExtendedObjectsReader extends AbstractSearchPathJdbcReader<MsJdbcLoader> implements IMsJdbcReader {
     ...
     */
    public MsExtendedObjectsReader(MsJdbcLoader loader) {
        super(loader);
    }
    ...
    protected void processResult(ResultSet res, ISchema schema) throws SQLException, XmlReaderException {
        ...
        if (type == DbObjType.PROCEDURE) {
            ...
            if ("FT".equals(funcType)) {
                setReturns(localFunc, MsXmlReader.readXML(res.getString("cols")));
            } else {
                localFunc.setReturns(MsJdbcLoader.getMsType(localFunc, res.getString("return_type_sh"),
        ...
        addArguments(func, MsXmlReader.readXML(res.getString("args")));
    ...
    private void setReturns(MsClrFunction localFunc, List<MsXmlReader> cols) {
        ...
        for (MsXmlReader col : cols) {
            ...
            column.setType(MsJdbcLoader.getMsType(localFunc, col.getString("st"), col.getString("type"),
    ...
    private void addArguments(MsAbstractClrFunction func, List<MsXmlReader> args) {
        for (MsXmlReader arg : args) {
            boolean isUserDefined = arg.getBoolean("ud");
            Argument argDst = new Argument(arg.getBoolean("ou") ? ArgMode.OUTPUT : ArgMode.IN,
                    arg.getString("name"), MsJdbcLoader.getMsType(func, arg.getString("st"),
